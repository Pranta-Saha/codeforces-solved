...
ll even[100005],odd[100005],oddsum=0,k,p,pp;

class solution{
public:
        ll n,m,i,j,oddi=0,eveni=0;
        void solve()
        {
                cin>>n>>k>>p; pp=p;
                for(i=0;i<n;i++)
                ...
                if(((k-p)%2!=oddsum%2) || ((k-p)>oddi) || k>n || ((oddi-(k-p))/2+eveni)<p )
                        { cout<<"NO"; return;}
                cout<<"YES"<<endl;
                if(p==0)
                {
                ...
                for(i; i<oddi && pp>1 ;i+=2)
                {
                        cout<<2<<' '<<odd[i]<<' '<<odd[i+1]<<endl;
                        pp--;
                        if(pp==1 || i+1==oddi-1) {i+=2; break;}
                }
                if(i>oddi) i=oddi;

                for(j=0;j<eveni && pp>1 ; j++)
                {
                        cout<<1<<' '<<even[j]<<endl;
                        pp--;
                        if(pp==1 || j==eveni-1) {j++; break;}
                }
                if(j>eveni) j=eveni;
