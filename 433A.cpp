...
int dp[101][201][201];
int n;
bool fun(int i,int tk,int os)
{
        if(i==n && tk==os) return true;
        if(i==n && tk!=os) return false;
        if(dp[i][tk][os]!=0) return dp[i][tk][os]-1;
        dp[i][tk][os] =  (fun(i+1,tk+arr[i],os) || fun(i+1,tk,os+arr[i])) +1;
        return  dp[i][tk][os]-1;
}
