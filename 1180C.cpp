...
int main()
{
        ...
        deque<ll>dq;
        vector<pii>vec;
        ll n,m,q,i,j,tmp,start,indxofmx,mx=INT_MIN;
        cin>>n>>q;
        for(i=0;i<n;i++)
        {
                cin>>j;
        ...
        vec.push_back({0,0});
        for(i=1;i<=indxofmx;i++)
        {
                ll a=dq.front(); dq.pop_front();
                ll b=dq.front(); dq.pop_front();
                if(a>b) {dq.push_back(b); dq.push_front(a);}
        ...
        {
                cin>>tmp;
                if(tmp<=indxofmx) {cout<<vec[tmp].first<<' '<<vec[tmp].second<<endl; continue;}
