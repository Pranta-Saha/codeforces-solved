...
ll n,m;
map<Tiii,int>dp;
ll go(ll a, ll b, ll level)
{
        Tiii tmp=Tiii(a,b,level);
        ...
        if(a<b)
        {
                if(b%2==0) x = go(a,b/2,level+1);
                if(b%3==0) y= go(a,b/3,level+1);
                if(b%5==0) z= go(a,b/5,level+1);
        }
        else
        {
                if(a%2==0) x = go(a/2,b,level+1);
                if(a%3==0) y= go(a/3,b,level+1);
                if(a%5==0) z= go(a/5,b,level+1);
        }
        dp[tmp]=min(x,y,z);
        return min(x,y,z);
}
