...
int main()
{
        ...
        int a,b,fruts_in_ith_day[3003]={0};
        cin>>n>>v;
        while(n--)
        {
                cin>>a>>b;
                fruts_in_ith_day[a]+=b;
        }
        int remining=0;
        for(i=1;i<=3001;i++)
        {
                tcs+=min(v,remining+fruts_in_ith_day[i]);
                remining=  max(0, fruts_in_ith_day[i] - max(0,min(v,remining+fruts_in_ith_day[i])-remining));
        }
        cout<<tcs;
