...
public:
        ...
        void go(int remain_x, int remain_y, int aa, int bb)
        {
                if(remain_x<0 || remain_y<0) return ;
                pii tmp={remain_x,remain_y};
                if(xy.find(tmp)!=xy.end()) return;
                else xy.insert(tmp);

                if(aa>bb) st.insert({aa,bb});
                go(remain_x-1,remain_y,aa+1,bb);
                go(remain_x-1,remain_y,aa,bb);
                go(remain_x,remain_y-1,aa,bb+1);
                go(remain_x,remain_y-1,aa,bb);
        }
