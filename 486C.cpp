...
public:
        int n,m,i,j,p;
        string str;
        int left_start=0,left_end,right_start,right_end,ans=0;
        int first_occur=-1,last_occur=-1;
        void solve()
        {
                cin>>n>>p>>str;
                n--; p--;
                left_end= (n&1)? n/2:n/2-1;
                right_start = n/2+1;
                ...
                if(p>left_end && first_occur!=-1) {int tmp=first_occur; first_occur=n-last_occur; last_occur=n-tmp; }

                if(first_occur!=-1)
                        ans=min(abs(p-first_occur),abs(p-last_occur))+abs(first_occur-last_occur);

                for(i=0;i<=left_end;i++)
                {
                        ans+=min( abs( str[i]-str[n-i] ), 26-abs( str[i]-str[n-i] )) ;
                }
                cout<<ans;
        }
};
